package com.example.text.myapplication.view.pulltorefresh;
...
import com.example.text.myapplication.view.pulltorefresh.internal.LoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	static final int PULL_TO_REFRESH = 0x0;//拉拽刷新
	static final int RELEASE_TO_REFRESH = 0x1;//释放刷新
	static final int REFRESHING = 0x2;//正在刷新
	static final int MANUAL_REFRESHING = 0x3;//手动刷新
	...
	private boolean isBeingDragged = false;//是否存在拖动

	private int state = PULL_TO_REFRESH;
	private int mode = MODE_PULL_DOWN_TO_REFRESH;
	private int currentMode;

	private boolean disableScrollingWhileRefreshing = true;//滑动的时候禁止刷新
	...
	//改造的方法将上拉刷新方法改造为加载更多
	private boolean isLoadMore;//是否可以加载更多，在加载更多的模式下,将上拉刷新的文字改为加载更多
	...
	public void setLoadMore(boolean loadMore) {
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			String pullLabel = getContext().getString(R.string.pull_load_more_pull_down_label);
			String refreshingLabel = getContext().getString(R.string.pull_load_more_refreshing_label);
			String releaseLabel = getContext().getString(R.string.pull_load_more_release_label);
			footerLayout.setPullLabel(pullLabel);
			footerLayout.setReleaseLabel(refreshingLabel);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mode = a.getInteger(R.styleable.PullToRefresh_refresh_mode, MODE_PULL_DOWN_TO_REFRESH);//mode = 0x3;
